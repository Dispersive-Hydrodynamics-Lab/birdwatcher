...
\frame{%
    ...
    \begin{enumerate}
        \item Conduit environment and equations that govern.
        \item Solitons, their properties, how they interact.
        \item Definition of soliton gas, its properties.
        \item Testing through simulations.
        \item Recurrence implications.
        \item Measuring a ``gas metric''.
        \item Running in parallel.
...
\frame{%
    ...
    \begin{itemize}
        \item Soliton gas theory has been developed for simpler (integrable)
            systems such as the Korteweg-de Vries equation (KDV)
            \begin{align*}
                \phi_t + \phi_{xxx} - 6 \phi \phi_x = 0
            \end{align*}
        \item Two main results
            \begin{itemize}
                \item There exists a compound Poisson process for the soliton centers and amplitudes
                    \begin{itemize}
                        \item This means over long time, $Z \sim
                            Poisson(\lambda)$, and A is preserved
                    \end{itemize}
                \item A soliton passing through the gas has a correction to its
                    speed that depends on the amplitude distribution of the gas
            \end{itemize}
    \end{itemize}
}
...
\frame{%
    ...
    \begin{itemize}
        ...
        \item At each timestep we'll check for a compound Poisson gas process
            (``gas metric'') of each. If they differ by more than 20\%, we
            restart with new initial conditions.
        \item Similarly, due to phase shifts, our gas metric should increase and
...
\frame{%
    ...
    \begin{itemize}
        ...
        \item We use the residual sum squared on the QQ-plot as a metric of
            ...
            gas metric.
        \item We find this gas metric at every timestep.
    \end{itemize}
}
...
\frame{%
    ...
    \item After a restart on $2L$ and $4L$ things are trickier... We can't feed
        initial conditions where solitons overlap (makes solver grumpy due to
        nonlinear phenomena) so instead we need to simulate initial conditions
        with same gas metric as last timestep before we quit.
    \end{itemize}
}
...
\frame{%
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[scale=0.38]{./img/results.png}
    \end{figure}
}
